...
target = "infection_next10"

def include_upto_infection(group):
    if pd.notnull(group['infection_day']).any():
        # Get the first infection day
        first_infection_day = group['infection_day'].min()
        return group[group['day'] <= first_infection_day]
    else:
        return group  # no infection, keep all

merged_df_orig = merged_df_orig.groupby('id', group_keys=False).apply(include_upto_infection)
...
RUN_NAME = 'infection_next10_sofaasv_nested_logit'
pred = Prediction(
    X=X,
    md=y,
    run_name=RUN_NAME,
